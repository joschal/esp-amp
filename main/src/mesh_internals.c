#include <string.h>
#include <include/amp_outbound.h>
 ...
 *******************************************************/
static const char *LOG_TAG = "mesh_internal";
static const uint8_t MESH_ID[6] = {0x42, 0x42, 0x42, 0x42, 0x42, 0x42};
...
void mesh_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {
	...
	switch (event_id) {
		case MESH_EVENT_STARTED: {
			esp_mesh_get_id(&id);
			ESP_LOGI(LOG_TAG, "<MESH_EVENT_MESH_STARTED>ID:"
					MACSTR
					"", MAC2STR(id.addr));
		...
		case MESH_EVENT_CHILD_CONNECTED: {
			...
			ESP_LOGI(LOG_TAG, "<MESH_EVENT_CHILD_CONNECTED>aid:%d, "
					MACSTR
					"",
			         child_connected->aid,
			...
			// process the event
			neighbour_connected((mesh_addr_t *) child_connected->mac);
		}
			break;
		case MESH_EVENT_CHILD_DISCONNECTED: {
			mesh_event_child_disconnected_t *child_disconnected = (mesh_event_child_disconnected_t *) event_data;
			ESP_LOGI(LOG_TAG, "<MESH_EVENT_CHILD_DISCONNECTED>aid:%d, "
					MACSTR
					"",
			...
			// process the event
			neighbour_disconnected((mesh_addr_t *) child_disconnected->mac);
			revoke_address_pool((mesh_addr_t *) child_disconnected->mac);
		}
			break;
		case MESH_EVENT_ROUTING_TABLE_ADD: {
			mesh_event_routing_table_change_t *routing_table = (mesh_event_routing_table_change_t *) event_data;
			ESP_LOGW(LOG_TAG, "<MESH_EVENT_ROUTING_TABLE_ADD>add %d, new:%d",
		...
		case MESH_EVENT_ROUTING_TABLE_REMOVE: {
			mesh_event_routing_table_change_t *routing_table = (mesh_event_routing_table_change_t *) event_data;
			ESP_LOGW(LOG_TAG, "<MESH_EVENT_ROUTING_TABLE_REMOVE>remove %d, new:%d",
			         routing_table->rt_size_change,
			         routing_table->rt_size_new);
		}
			break;
		...
		case MESH_EVENT_PARENT_CONNECTED: {
			...
			mesh_connection_init(mesh_parent_addr);
		...
		case MESH_EVENT_LAYER_CHANGE: {
			...
			ESP_LOGE("PARENT layer change before", "");
			mesh_connection_init(mesh_parent_addr);

			ESP_LOGE("PARENT layer change after", "");
		}
			break;
		case MESH_EVENT_ROOT_ADDRESS: {
			mesh_event_root_address_t *root_addr = (mesh_event_root_address_t *) event_data;
			ESP_LOGI(LOG_TAG, "<MESH_EVENT_ROOT_ADDRESS>root address:"
					MACSTR
					"",
		...
		case MESH_EVENT_ROOT_SWITCH_ACK: {
			...
			ESP_LOGI(LOG_TAG, "<MESH_EVENT_ROOT_SWITCH_ACK>layer:%d, parent:"
					MACSTR
		...
		case MESH_EVENT_FIND_NETWORK: {
			...
			ESP_LOGI(LOG_TAG, "<MESH_EVENT_FIND_NETWORK>new channel:%d, router BSSID:"
					MACSTR
					"",
		...
		case MESH_EVENT_ROUTER_SWITCH: {
			...
			ESP_LOGI(LOG_TAG, "<MESH_EVENT_ROUTER_SWITCH>new router:%s, channel:%d, "
					MACSTR
					"",
...
void ip_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {
	...
	ESP_LOGI(LOG_TAG, "<IP_EVENT_STA_GOT_IP>IP:"
...
void init_mesh(void) {
	...
	ESP_LOGI(LOG_TAG, "mesh starts successfully, heap:%d, %s\n", esp_get_free_heap_size(),
...
void send_amp_message(union amp_msg *msg) {
	...
	mesh_addr_t *addr = get_layer_2_address(msg);

	send_amp_message_via_layer_2(msg, addr);
...
void send_amp_message_via_layer_2(union amp_msg *msg, mesh_addr_t *destination) {
	...
	if (destination == NULL) {
		mesh_addr_t invalidAddress = {{0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF}};
		flood_amp_msg(msg, invalidAddress);
		return;
	}

	mesh_data_t *meshData = malloc(sizeof(mesh_data_t));
	meshData->data = (uint8_t *) &msg->buffer;
	meshData->size = AMP_MESSAGE_SIZE;
	meshData->proto = MESH_PROTO_BIN;
	meshData->tos = MESH_TOS_P2P;

	ESP_LOGI("TX", "Sending %#X to "
			MACSTR, msg->generic_msg.header.msg_type, MAC2STR(destination->addr));
